import { formatNumber, METRIC_FIELD, METRIC_LABELS, type FinancialsVM, type Metric } from "@lasso/spec";
...
export function GroupedBarChart({
  ...
  const isCount = shownMetrics.every((m) => m === "ansatte");
  const all = points.flatMap((p) => p.values);
  const { scale, label } = labelFor(all, isCount);
