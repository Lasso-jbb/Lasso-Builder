...
| **3. Nye komponenter** | `LassoFinancialStatement` (19, data findes i XBRL) · `LassoMultiYearTable` + `LassoGauge` (10) · flere graftyper i en fælles `LassoChart` (grupperet, linje + område, stablet, donut, vandfald, rangliste) (13) · `LassoRiskObservations` (17, kobl `/modules/observations` på) · `LassoOwnershipDiagram` (14, 14B, kræver ejergraf-API og en layout-algoritme i lag med foldede kæder og cirkulært ejerskab) | 8–14 t |
...
| **Sonnet 5** | Trin 3 (undtagen ejerdiagram), responsivt og rettelser efter visuel test. Flere agenter parallelt. | 8–12 t |
...
2. Parallelt i ca. 4–8 t: Sonnet-agenter bygger nye komponenter, mens Opus laver ejerdiagrammet.
3. Afslutning i ca. 5–6 t.

Samlet ca. 16–24 t, før menneskelige gennemsyn kommer oveni. Det er gennemsynene, der bestemmer den reelle kalendertid.

**Største usikkerheder:**
- Ejerdiagrammet (4–8 t).
- Adgang til data for kredit, nyheder, person, BBR/CHR og overvågning. Hver datatype tager 1–3 t, når der er endpoints.

## 8. Åbne spørgsmål, der skal afklares

1. Endpoint og adgang til ejergraf-API'et (docs.lassox.com/module-apis/ownergraph).
2. Kilde til kreditvurdering: Creditsafe eller `/modules/valuations`, som i dag svarer tomt.
3. Endpoints til nyheder, personer, P-enheder, BBR, CHR og overvågning.
4. Skal portalens elementer (navigation, dialoger, A4-eksport) med senere?
5. Må gamle komponentnavne omdøbes? Hvis ja, skal gemte visninger migreres.
